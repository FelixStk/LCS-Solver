...
class MatrixConstIterTest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(MatrixConstIterTest, ForwardTraversal) {
...
TEST_F(MatrixConstIterTest, BackWardTraversal) {
  std::vector<std::vector<size_t>> axis_orders;
  axis_orders.push_back({0, 1, 2});
  axis_orders.push_back({1, 0, 2});
  axis_orders.push_back({0, 2, 1});
...
TEST_F(MatrixConstIterTest, EqualityAndInequality) {
...
// Test dereferencing
TEST_F(MatrixConstIterTest, Dereferencing) {
  std::vector<size_t> axis_order = {1, 0, 2};
...
TEST_F(MatrixConstIterTest, PostIncrement) {
  std::vector<size_t> axis_order = {1, 0, 2};
  MatrixConstIter<int> iter(matrix_, axis_order);
...
TEST_F(MatrixConstIterTest, EndIterator) {
...
TEST_F(MatrixConstIterTest, InvalidAxisOrder) {
