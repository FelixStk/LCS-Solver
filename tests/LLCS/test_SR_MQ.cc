...
//==== Initialize Test Suites ==================================================
 class LLCS2_SR_MQ_Test : public ::testing::TestWithParam<AlgoParam> {};

INSTANTIATE_TEST_SUITE_P(
    HardCodedParameters, LLCS2_SR_MQ_Test,
    ValuesIn(genHardCodedParams(ConstraintType::SIGMA_R)),
    [](const ::testing::TestParamInfo<LLCS2_SR_MQ_Test::ParamType> &info) {
      return info.param.name;
    });

INSTANTIATE_TEST_SUITE_P(
    UniqueSolParameters, LLCS2_SR_MQ_Test,
...
INSTANTIATE_TEST_SUITE_P(
    StdRelaxedParameters, LLCS2_SR_MQ_Test,
...
TEST_P(LLCS2_SR_MQ_Test, SolutionEq) {
  auto &[name, spv, map, sol] = GetParam();
  const auto N = spv.size();
  auto algo = std::make_unique<LLCS2_SR_MQ>(spv, map);
