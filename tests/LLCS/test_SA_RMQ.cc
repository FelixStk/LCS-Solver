 ...
 * @version 1.3
 * @brief GTests for the correctness of the LLCS2_SA_RMQ algorithm
 ******************************************************************************/
...
//==== Initialize Test Suites ==================================================
 class LLCS2_S_RMQ_Test : public ::testing::TestWithParam<AlgoParam> {};

INSTANTIATE_TEST_SUITE_P(
    HardCodedParameters, LLCS2_S_RMQ_Test,
    ValuesIn(genHardCodedParams(ConstraintType::SIGMA)),
    [](const ::testing::TestParamInfo<LLCS2_S_RMQ_Test::ParamType> &info) {
      return info.param.name;
    });

INSTANTIATE_TEST_SUITE_P(
    UniqueSolParameters, LLCS2_S_RMQ_Test,
...
INSTANTIATE_TEST_SUITE_P(
    StdRelaxedParameters, LLCS2_S_RMQ_Test,
...
TEST_P(LLCS2_S_RMQ_Test, SolutionEq) {
  auto &[name, spv, map, sol] = GetParam();
  const auto N = spv.size();
  auto algo = std::make_unique<LLCS2_SA_RMQ>(spv, map);
