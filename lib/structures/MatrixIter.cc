...
template<typename T>
typename MatrixIter<T>::reference MatrixIter<T>::operator*() const {
  if (!matrix_) {
...
template<typename T>
typename MatrixIter<T>::pointer MatrixIter<T>::operator->() const {
  if (!matrix_) {
    throw std::runtime_error("Dereferencing a default-constructed iterator.");
  }
  return &(*matrix_)[linear_index_];
...
template<typename T>
MatrixIter<T> MatrixIter<T>::operator++(int) {
  MatrixIter temp = *this;
  ++(*this);
  return temp;
}
...
// Equality operators
template<typename T>
bool MatrixIter<T>::operator==(const MatrixIter &other) const {
...
template<typename T>
void MatrixIter<T>::incrementIndices() {
  if (!matrix_) {
    throw std::runtime_error("Incrementing a default-constructed iterator.");
  }
...
template<typename T>
void MatrixIter<T>::decrementIndices() {
  if (!matrix_) {
...
template<typename T>
std::string MatrixIter<T>::DebugString() const {
...
template class MatrixIter<int>;
template class MatrixIter<util::uint>;
